...
std::vector<ME0StubPrimitive> l1t::me0::process_partition(const std::vector<UInt192>& partition_data,
    ...
    */
    ...
    std::vector<ME0StubPrimitive> segments = pat_mux(partition_data, partition_bx_data, partition, config);
    ...
    std::vector<std::vector<ME0StubPrimitive>> chunked = chunk(segments, config.group_width);
