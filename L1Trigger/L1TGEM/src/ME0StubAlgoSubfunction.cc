...
std::vector<ME0StubPrimitive> l1t::me0::PeakingManager::processSegments(const int partition, const std::vector<ME0StubPrimitive>& newSegs) {
  auto trig = trigger[partition];
  auto oldSegs = segs[0][partition];
  auto oldestSegs = segs[1][partition];
  ...
  for (size_t i = 0; i < trig.size(); ++i) {
    if (trig[i]) {
      ...
      trigger[partition][i] = false; // reset trigger after firing
    } else if (oldSegs[i].LayerCount() > 0 && oldestSegs[i].LayerCount() == 0) {
      if (newSegs[i].LayerCount() == 0) {
        output.push_back(oldSegs[i]); // trigger stays false as old segment is still existed while new segment is not existed
      } else {
        output.push_back(ME0StubPrimitive());
        trigger[partition][i] = true; // set trigger as new segment and old segment are existed while oldest segment is not existed
      }
    } else {
      output.push_back(ME0StubPrimitive());
    }
  ...
  // Update segs
  segs[1][partition] = oldSegs;
  segs[0][partition] = newSegs;
...
std::pair<int, std::vector<int>> l1t::me0::findCentroid(uint64_t& data) {
  ...
  double resolutionFactor = 2.0; // 1.0 = single strip resolution, 2.0 = half strip resolution, ...; FW is bit-retricted, so this must match the FW implementation
