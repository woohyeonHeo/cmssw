...
namespace l1t {
  namespace me0 {
    ...
    class PeakingManager {
      private:
        std::vector<std::vector<std::vector<ME0StubPrimitive>>> segs; // [oldest / old][partition][sbit] : size = (2, 15, 192)
        std::vector<std::vector<bool>> trigger; // [partition][sbit]
      public:
        PeakingManager() = default;
        PeakingManager(const int numPart = 15, const int width = 192) {
          segs = std::vector<std::vector<std::vector<ME0StubPrimitive>>>(2, std::vector<std::vector<ME0StubPrimitive>>(numPart, std::vector<ME0StubPrimitive>(width)));
          trigger = std::vector<std::vector<bool>>(numPart, std::vector<bool>(width, false));
        }
        std::vector<ME0StubPrimitive> processSegments(const int partition, const std::vector<ME0StubPrimitive>& newSegs);
        std::vector<std::vector<bool>> getTrigger() const { return trigger; };
        std::vector<std::vector<std::vector<ME0StubPrimitive>>> getSegs() const { return segs; };
    };
    ...
    struct Config {
      ...
      void initializePeakingManager() {
        peakingManager = PeakingManager(width, (xPartitionEnabled ? 15 : 8));
      }
      std::vector<int32_t> patSpans;
      void initializePatSpans() {
        patSpans.reserve(kPatternDefinitions.size());
        for (const auto& pattern : kPatternDefinitions) {
          int high = std::max(pattern.layers[0].hi, pattern.layers[5].hi);
          int low = std::min(pattern.layers[0].lo, pattern.layers[5].lo);
          patSpans.push_back(high - low + 1);
        }
      }
      Config() { initializePatSpans(); if (peakingEnabled) initializePeakingManager(); }
    };
